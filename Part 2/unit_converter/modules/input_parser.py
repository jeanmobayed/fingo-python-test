...
from unit_converter.modules.roman_numerals import RomanNumerals
...
class InputParser:
        ...
        for item in ref_words:
            ...
            if len(patterns) == 3 and patterns[1] == 'is':
                if patterns[0] not in self.words_book:
                    ...
                    #print("update Word " + patterns[0] + " from " + self.words_book[patterns[0]] + " to " + patterns[2])
                    self.words_book.update({patterns[0]: patterns[2]})
                else:
        ...
        for item in prices_words:
            ...
            if amount_arabic:
                ...
                if good_name not in self.prices_book:
                    ...
                    #print("update price of " + good_name + " from " + self.prices_book[good_name]+ " to " + good_price)
                    self.prices_book.update({good_name: good_price})
                else:
                    # duplicated message
                    continue
    ...
    def clasify_input(self, input_array):
        """
    ...
    def get_price_book(self):
        return self.prices_book
    
    def get_words_book(self):
        return self.words_book
